...
def app():
    ...
    if uploaded_file is not None:
        ...
        st.write(data)
        group_col = st.selectbox("Select the column for grouping (categorical):", data.columns.str.strip())
        value_col = st.selectbox("Select the column for values (numerical):", data.columns.str.strip())

        if st.button("Perform ANOVA analysis"):
            # Perform ANOVA analysis for the uploaded data
            groups = data[group_col].unique()
            anova_data = [data[data[group_col] == g][value_col].values for g in groups]
            ...
            fig_data = px.violin(data_frame=data, x=group_col, y=value_col, box=True, points="all", title="Violin Plot")
            fig_data.update_layout(autosize=True, margin=dict(l=0, r=0, t=30, b=0), width=None)
            st.plotly_chart(fig_data, use_container_width=True)
            img_base64_data = convert_fig_to_base64(fig_data)
            st.download_button(
